        ...
        if not reverse:
            for layer in self.layers:
                if isinstance(layer,modules.Split):
                    z, logdet, logpz = layer(z, logdet=logdet, logpz=logpz, eps=eps, reverse=False)
                    ...
                    z, logdet, logpz = layer(x=z, logdet=logdet, logpz=logpz, eps=eps, reverse=False)

        else:
            # Decode
            for layer in reversed(self.layers):
                    if isinstance(layer, modules.Split):
                        z, logdet, logpz = layer(z, logdet=logdet,logpz=logpz, eps=eps, reverse=True)
                    elif isinstance(layer, FlowStep):
                        z, logdet = layer(z, logdet=logdet, reverse=True)
                    elif isinstance(layer, modules.Squeeze):
                        z = layer(z, reverse=True)
                    elif isinstance(layer, modules.GaussianPrior):
                        z, logdet, logpz = layer(x=z, logdet=logdet, logpz=logpz, eps=eps, reverse=True)
        
        return z, logdet, logpz

class FlowModel(nn.Module):
    def __init__(self,input_shape,
                 filter_size, L, K, bsz, n_bits_x=8):
        super().__init__()
    ...
    def forward(self, x_image=None, z=None, logdet=0, reverse=False, use_stored=False, eps=None):
    ...
    def _dequantize(self, x, n_bins):
        ...
        unif_noise = torch.zeros_like(x).uniform_(0, 1. / n_bins)
        x = unif_noise + x
