...
def vis_eval_test_images(y_hat, title, args, eps=0):
        os.makedirs('runs/{}/snapshots/test_images/{}/'.format(args.exp_name,args.testset), exist_ok=True)

        # todo: check how to get image file name form dataloader
        savedir = 'runs/{}/snapshots/test_images/{}/{}_eps{}.png'.format(args.exp_name, args.testset, title, eps)
...
def vis_eval_plotting(x, y, exp_name, logging_step, plot_type, eps=0):
    ...
    if plot_type == 'reconstruction':
        savedir = 'runs/{}/snapshots/step{}_reconstruction.png'.format(exp_name, logging_step)
        right_title = 'y_tilde'
        left_title = 'y'
        ...
        savedir = 'runs/{}/snapshots/step{}_mu+{}.png'.format(exp_name, logging_step, eps)
...
def plot_samples_test(n, z_sample, x_lr, modelname, datapart, plot_type=None, grid=False):
    if not os.path.exists('snapshots/super_resolved/'):
        os.makedirs('snapshots/super_resolved/')

    savedir = 'snapshots/super_resolved/{}_super_resolved_{}.png'.format(datapart, modelname)
    ...
    if not grid:
        ...
        for i in range(n):
            ...
            #plt.axis('off')
            plt.imshow(grid2.permute(1, 2, 0).contiguous())
            savedir = 'snapshots/super_resolved/{}_{}_super_resolved_{}.png'.format(i, datapart, modelname)
            #plt.axis('off')
            plt.savefig(savedir)
