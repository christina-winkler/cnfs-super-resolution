...
class Gaussian_Diag(object):
    ...
    def logp(self, x, mean, sigma):
        ...
        ll = -0.5 * (x - mean)**2 / (sigma**2) - 0.5 * torch.log(2 * np.pi * ( sigma**2) * ones)
...
class DiscLogistic(object):
    ...
    def log_pmass(self, y, means, logsigmas, num_classes, log_scale_min=-7.0):
        ...
        plus_in = inv_std * (centered_y + 1. / num_classes)
        cdf_plus = torch.sigmoid(plus_in)
        min_in = inv_std * (centered_y - 1. / num_classes)
        cdf_min = torch.sigmoid(min_in)

        max_condition = (y > 0.9999 - 1 / 256.).float()
    ...
    def sample_logistic(self, means, logscales, 
                        log_scale_min=-7.0,
        ...
        u = means.data.new(means.size()).uniform_(1., 0)
        scale = torch.exp(logscales)
        sample = means + scale * torch.log(u / (1. - u))
        sample = sample.clamp(min=0, max=float(256. - 1.) /
                         float(256.))
        return sample, means